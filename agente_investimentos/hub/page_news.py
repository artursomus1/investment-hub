...
from agente_investimentos.ai_engine.gemini_client import analyze_daily_summary_ai
from agente_investimentos.dashboard.session_persistence import save_daily_summary_text, load_daily_summary_text
...
def _parse_date(date_str: str) -> datetime | None:
    """Tenta parsear a data do RSS (formato RFC 2822)."""
    if not date_str:
        return None
    # Formatos comuns do Google News RSS
    for fmt in (
        "%a, %d %b %Y %H:%M:%S %Z",
        "%a, %d %b %Y %H:%M:%S %z",
        "%Y-%m-%dT%H:%M:%SZ",
...
def _render_daily_summary_section(news_data: dict):
    """Renderiza a secao de resumo diario com botao de geração."""
    st.subheader("Resumo do Dia")

    # Carrega resumo salvo se existir
    if "daily_summary_text" not in st.session_state:
    ...
    with col_info:
        st.caption("Resumo via IA focado em Geopolítica, Inteligencia Artificial e Economia")

    if generate:
        all_news = get_all_news_flat(news_data)
        with st.spinner("Gerando resumo diario via Gemini..."):
            summary_text = analyze_daily_summary_ai(all_news)
            if summary_text.startswith("[ERRO]"):
                ...
                save_daily_summary_text(summary_text)

    summary_text = st.session_state.get("daily_summary_text")
